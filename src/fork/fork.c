...
/*   Updated: 2022/02/08 16:00:48 by adoner        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void first_child(t_pipeline *pip_line, t_list *env, char **envp, int fd[2])
{
	...
	if (id == 0)
	{
		...
		// if (pip_line->redirection->redir_type!= NONE)
		// {
...
void middle_child(t_pipeline *pip_line, t_list *env, char **envp, int fd[2], int endfile)
{
	int id;
...
void last_child(t_pipeline *pip_line, t_list *env, char **envp, int fd[2], int *lastid)
{
	*lastid = fork();
	if (*lastid == 0)
	{
